...
/*
 * Запасной ключ поиска цитаты (её начало) берётся только когда встречается
 * один раз. Для русского техтекста короткий ключ — это «Требования к
 * проектной документации», и по первому совпадению правка вставала бы
 * в чужой пункт (остаточный риск, названный рецензией 10.09.2026).
 */
test('анализ ТЗ: неоднозначная цитата уходит в дополнения, а не в первый похожий пункт', () => {
  const revision = require('../server/services/tz/revision');
  const text = [
    '1. Требования к проектной документации по разделу ОВ: состав и объём.',
    '2. Требования к проектной документации по разделу ЭОМ: состав и объём.',
    '3. Требования к проектной документации по разделу ВК: состав и объём.',
  ].join('\n\n');
  // цитата обрезана моделью так, что её полного вхождения в тексте нет,
  // а начало подходит ко всем трём пунктам
  const findings = [{ id: 'F-050', quote: 'Требования к проектной документации по разделу ЭОМ: состав, объём и порядок' }];
  const fixes = [{ findingId: 'F-050', chosenText: 'Дополнить состав раздела ЭОМ.', chosenKind: 'own', authorName: 'Инженер' }];
  const out = revision.build(text, findings, fixes, '');
  assert.equal(out.applied, 1);
  assert.equal(out.byFinding[0].placed, 'в дополнения', 'правка встала в пункт, выбранный наугад');
  assert.ok(out.text.includes(revision.HEAD), 'нет раздела дополнений');
  // ни один исходный пункт не разорван вставкой
  for (const line of text.split('\n\n')) {
    const at = out.text.indexOf(line);
    assert.ok(at >= 0, `потерян пункт: ${line.slice(0, 30)}`);
  }
});
