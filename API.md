...
## Разбор PDF по форматам — `/api/print`

Отдельная вкладка вне проектов (`/print.html`). Детерминированно, моделей нет:
poppler читает боксы и `/Rotate` каждой страницы, код классифицирует лист по
ГОСТ 2.301-68 (основной → кратный → плюсовой → НС) и назначает носитель по
короткой стороне (лист принтера или рулон плоттера). На каждую корзину
собирается один PDF-пакет, который оператор открывает во вкладке и печатает.
Все запросы, кроме билета, — с `X-User-Token`. Разбор у человека один: новый
стирает предыдущий; срок хранения — `PRINT_TTL_HOURS` (24 ч).

| Метод | Путь | Что делает |
|---|---|---|
| GET | `/park` | парк носителей и таблицы форматов (`park.json`), настройки по умолчанию, лимиты (`chunkBytes`, `maxFileBytes`, `maxTotalBytes`, `maxFiles`, `ttlHours`, `ticketMinutes`), `poppler: true/false` |
| GET | `/jobs` | текущий разбор человека (`job` или `null`) |
| POST | `/jobs` | новый разбор (прежние удаляются) → 201 `{ job }` |
| GET | `/jobs/:id` | состояние: `status` (new · running · done · error · cancelled · interrupted), `progress {phase, done, total, label}`, `files`, `summary`, `packages`, `problems`; `?records=1` у готового разбора добавляет `records` — строку на каждый лист |
| DELETE | `/jobs/:id` | удалить разбор со всеми файлами |
| POST | `/jobs/:id/files` | JSON `{ name, size }` → 201 `{ file: { id, chunks, chunkSize, … } }`; только `.pdf`, потолки по лимитам |
| PUT | `/jobs/:id/files/:fid/chunks/:n` | тело `application/octet-stream` — кусок `n` ровно `chunkSize` байт (последний — остаток); пишется по смещению, повтор и перестановка безвредны; `complete: true` у файла, когда собраны все |
| DELETE | `/jobs/:id/files/:fid` | убрать файл; прежний результат сбрасывается |
| POST | `/jobs/:id/run` | JSON настроек: `tolerance` (мм, 0…20, по умолчанию 3), `box` (crop · media · trim · art), `scanOnly`, `multiplesOwn`, `plusOwn`, `plusRolls` → 202; 409, если не все файлы дособраны или разбор уже идёт |
| POST | `/jobs/:id/cancel` | остановить: исходники остаются, пакеты не собираются |
| GET | `/jobs/:id/report.csv` | отчёт: `;`, UTF-8 с BOM, строка на лист (файл, страница, габариты, Rotate, бокс, класс, формат, корзина, носитель, рулон, отрез, пакет и страница в нём, имя листа по ТЗ, примечание) |
| GET | `/jobs/:id/report.json` | то же плюс настройки, сводка, пакеты и проблемы |
| POST | `/jobs/:id/tickets` | билет на `PRINT_TICKET_MINUTES`: `{ ticket, expiresAt, packages: [{ bucket, file, pages, bytes, url }] }` |
| GET | `/p/:ticket/:file` | пакет во вкладку **без** `X-User-Token`: `application/pdf`, `Content-Disposition: inline`, Range поддерживается; чужой, устаревший билет или имя не из списка пакетов — 404 |

Корзины: `А5`…`А0` (стандарт), `А4+`…`А0+` (плюсовые, по флагу — в `НС`),
`НС А4` / `НС А3` (лист принтера или рулон 297), `НС А2` (рулон 420), `НС А1`
(594), `НС А0` (841), `НС 96` (960), при `plusRolls` — `НС А3+` и т. д.;
`_ТРЕБУЕТ_РЕШЕНИЯ` — короткая сторона шире всех рулонов. Кратные форматы
ГОСТ по умолчанию идут в корзину своего рулона, `multiplesOwn` выносит их в
собственные пакеты (`А4х3`, …).
