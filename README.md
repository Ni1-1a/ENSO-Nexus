...
Отдельные вкладки вне проектов: «Виртуальный офис» (3D-витрина) и «Разбор PDF
по форматам» — листы комплекта по лоткам принтера и рулонам плоттера, пакет на
каждую корзину открывается во вкладке и печатается (poppler, без нейросети).
...
│   │                        # print.js — вкладка «Разбор PDF по форматам» (кусками, пакеты по билету)
│   └── services/
...
│       ├── print/           # разбор PDF по форматам: park.json, formats.js, pdfinfo.js, jobs.js
...
├── public/                  # фронтенд: index.html + tz/doccheck/normo/gge/akty.html + office/print.html, без сборки
