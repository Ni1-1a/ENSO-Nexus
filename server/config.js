...
const config = {
  ...
  /**
   * Сколько раз переспрашивать модель после ответа, не прошедшего проверку
   * схемы. Касса обращений (MAX_ANALYSIS_CALLS) общая с дочитыванием обрезанных
   * ответов, и без своего потолка повторы съедали её целиком: на локальной
   * модели с окном 72 тыс. токенов это 12 вызовов по шесть минут — полтора часа
   * одинаковых «материалы сокращены под контекст» (боевой прогон 09.09.2026).
   */
  maxAnalysisRetries: int('MAX_ANALYSIS_RETRIES', 4),
  /**
  ...
  /**
   * Провайдеры, выключенные владельцем совсем: их нет ни в пикере, ни в
   * статистике, ни в «остатках на счетах», а сохранённый в сессии выбор
   * получает отказ «провайдер отключён». Пустой ключ этого не даёт: провайдер
   * без ключа остаётся в списке с пометкой «нужен ключ» — это состояние
   * «ещё не настроен», а не «отключён». Пример: аккаунт OpenAI деактивирован
   * 10.08.2026, маршрут chatgpt мёртв — держать его в списке незачем.
   *
   *   AI_DISABLED_PROVIDERS=chatgpt
   */
  aiDisabledProviders: new Set(
