...
for (const sql of [
  ...
  // Имя платформы, с которого работают в проекте: enso-nexus.com или .ru.
  // Нужно гейту облака (`services/ai/cloud-access.js`): решение принимается на
  // дне адаптера, у самого вызова модели, где запроса уже нет — есть только
  // идентификатор сессии. Поле обновляется на каждом обращении к проекту,
  // поэтому переход человека с одного адреса на другой не застревает.
  "ALTER TABLE sessions ADD COLUMN origin_host TEXT DEFAULT ''",
