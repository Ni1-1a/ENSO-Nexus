...
const MODEL_RULES = [
  ...
  // qwen3.8 — мультимодальное поколение: зрение есть у всей линейки. Думающая:
  // размышления идут отдельным полем и тратят бюджет ответа. Строгая схема
  // ВЫКЛЮЧЕНА: MLX-движок LM Studio (mlx-llm 1.11.0) на архитектуре qwen3_5
  // завершает генерацию сразу после </think> с пустым текстом — замер
  // 2026-08-28, все режимы (strict, enable_thinking=false, reasoning_effort=low).
  // Без схемы модель отдаёт чистый JSON после размышлений, разбор и валидация
  // у адаптера свои. Перепроверить при обновлении mlx-движка или на GGUF-сборке.
  { provider: 'lmstudio', match: /qwen3\.8/i, on: { vision: true, reasoning: true, structuredOutput: false } },
...
const MODEL_NOTES = [
  ...
  {
    // qwen3.8 — и текст, и зрение в одной модели; ставится ВЫШЕ правила «-vl»
    // и общего среднего класса, иначе описание достанется не то.
    provider: 'lmstudio', match: /qwen3\.8/i, tier: 'рабочая, локальная',
    summary: 'Локальное поколение Qwen 3.8: текст и зрение в одной модели, документы не покидают машину.',
    strengths: [
      'Сканы и чертёжную графику читает сама — прогон не переключается на отдельную vision-модель',
      'Думающая: правила извлекает аккуратно, с основанием и без выдумок (замер 2026-08-28)',
      'Дешёвый KV-кэш — длинное окно достаётся без переполнения памяти',
    ],
    limits: [
      'Размышления тратят время и бюджет ответа — отвечает медленнее qwen3.5-35b-a3b',
      'Нормативы знает поверхностно — опирается на базу знаний, а не на память',
    ],
    bestFor: 'Основная локальная модель: разбор комплекта, ограничения, чат и сканы одной моделью.',
  },
  {
