 ...
 */
function analyse(site, buildable, req, { maxCandidateAreaM2 = 0, shapesTried = false } = {}) {
    ...
    measures.push({
      ...
      text: `Свободной площади достаточно (${availableM2} м² против ${neededM2} м² застройки), но ни одно `
        ...
        + (maxCandidateAreaM2 > 0
          ? `По сетке колонн (шаг 6 м, корпус не уже 12 м) здесь помещается не больше ${round(maxCandidateAreaM2)} м². `
          : '')
        // совет про формы уместен, только если формы НЕ перебирались: после
        // перебора Г/Т/П, ступенчатой и контура по сетке колонн он звучит как
        // издёвка — человек либо сам их запретил, либо они уже не помогли
        + (shapesTried
          ? 'Готовые формы (Г, Т, П, ступенчатая) и контур по сетке колонн уже перебраны — '
            + 'уменьшите площадь застройки, задайте меньший шаг колонн и ширину корпуса (gridStepM и minWidth вместе) '
            + 'или снимите часть ограничений.'
          : 'Помогут Г-, Т- или П-образная форма корпуса и другие пропорции — прямоугольник в такую площадку не ложится '
            + '(изменение формы сейчас запрещено: allowReshape).'),
    });
  }
