...
/*
 * Треугольник и трапеция из перебора УБРАНЫ (решение владельца 09.09.2026):
 * здание — это прямые углы, а острый клин площадки заполняется не косой
 * стеной, а ступенчатым контуром по сетке колонн (geometry/grid-shape.js).
 * На боевом проекте в Горбунках движок предлагал треугольное пятно, и человек
 * дважды писал замечание «все углы прямые» — а оно до кода не доходило:
 * координаты считает движок, и запрет надо было ставить здесь.
 * Подписи оставлены ради старых запусков, где такая форма уже сохранена.
 */
const LEGACY_LABELS = {
  trapezoid: { label: 'трапеция', note: 'повторяет сходящиеся границы участка (форма больше не предлагается)' },
  triangle: { label: 'треугольная', note: 'острый клин участка (форма больше не предлагается)' },
  grid: { label: 'по сетке колонн', note: 'ступенчатый контур из ячеек шага колонн, все углы прямые' },
};
...
function label(shapeId) {
  const s = BY_ID.get(shapeId) || LEGACY_LABELS[shapeId];
