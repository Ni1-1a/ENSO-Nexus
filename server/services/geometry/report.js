...
<section class="break-before">
  ...
  ${facts([
    ...
    // число углов — то, по чему заказчик судит о форме (решение владельца 09.09.2026);
    // у запусков до этой даты поля нет — строка не печатается
    ['Углов в контуре', Number.isFinite(m.corners) ? `${m.corners}${m.orthogonal === false ? ' (есть непрямые)' : ' (все прямые)'}` : null],
