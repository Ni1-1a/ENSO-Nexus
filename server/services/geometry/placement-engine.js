  ...
  if (!found.length) {
    ...
    const reason = tight
      ? `Здание не помещается: нужно ${neededM2} м² застройки, а свободно от ограничений ` +
        `всего ${availableM2} м². Уменьшите площадь застройки, поднимите этажность ` +
        'или пересмотрите ограничения — при таком соотношении посадки не существует.'
      : `Свободной площади хватает (${availableM2} м² против ${neededM2 || '?'} м² застройки), ` +
        ...
        'она слишком узкая или разрезана ограничениями. Помогут другая форма здания, ' +
        'другие пропорции или пересмотр ограничений.';
    return {
      candidates: [],
      errors: [],
      reason,
      tried,
      total: 0,
      warnings: [{ code: 'placement-empty', message: reason }],
    };
  }
