 ...
 * По умолчанию — методика «12 шагов посадки здания» (как в системном промпте).
 * Пользователь может скачать её как Excel и загрузить свой —
 * тогда анализ идёт по его шагам (блок <workplan> в промпте).
 *
...
function promptText(wp) {
  ...
  return `Пользователь загрузил собственный порядок анализа («${wp.name}»). ` +
    'Выполняй анализ СТРОГО по этим шагам и в этом порядке — ВМЕСТО стандартной методики «12 шагов». ' +
    'Разделы report_markdown — по этим шагам. Формат ответа (JSON-схема) не меняется.\n' + lines.join('\n');
}
